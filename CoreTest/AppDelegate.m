...
#include <sys/time.h>
#import "Zip/ZipArchive/ZipArchive.h"
...
-(BOOL)installEPub:(NSString*)fileName {
    ...
    // If you want to use SkyProvider/EPubProvider, you don't need to unzip files into file system.
    // then set unzipToLocalFileSystem = NO
    BOOL unzipToLocalFileSystem = YES;
    if (unzipToLocalFileSystem) {
        ...
        // creates the folder for storing cover image and cache data.
        [self createEPubDirectory:fileName];
    }
    
    BookInformation* bi= [[BookInformation alloc]initWithBookName:fileName baseDirectory:[self getBaseDirectory]];
...
// Since skyepub 5.0
// This function will prevent Documents folder from being backuped by iCloud.
-(void)preventDocuemntsBackup {
