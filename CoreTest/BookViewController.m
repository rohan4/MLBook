...
-(Theme*)getTheme {
    int themeIndex = self.setting.theme;
...
-(Theme*)getThemeByIndex:(int)index {
    if (index<0) index=0;
    if (index>[themes count]-1) index = (int)[themes count]-1;
    Theme* theme = [themes objectAtIndex:index];
    return theme;
}
...
-(void)makeBookViewer {
    @autoreleasepool {
        ...
        if ([self isPad]) {
            [rv setVerticalGapRatio:0.12]; // leaves the room for top and bottom icons.
        }else {
            [rv setVerticalGapRatio:0.20]; // leaves the room for top and bottom icons.
        }
        [rv setHorizontalGapRatio:0.3];
        [rv setGlobalPaging:setting.globalPagination];
        [rv showIndicatorWhileLoadingChapter:NO];
        ...
        // set Theme
        [self applyThemeToRV:self.setting.theme];
...
//        EPubProvider* epubProvider = [[EPubProvider alloc]init];
...
-(void)applyThemeToRV:(int)themeIndex {
    Theme* theme = [self getThemeByIndex:themeIndex];
...
-(void)applyThemeToUI:(int)themeIndex {
    ...
    [authorLabel setTextColor:theme.controlColor];
    [titleLabel setTextColor:theme.controlColor];
    [pageIndexLabel setTextColor:theme.controlColor];
    [secondaryIndexLabel setTextColor:theme.controlColor];
}

// When theme need to be changed without leaving BookViewController
// since skyepub 5.0
-(void)changeTheme:(int)themeIndex {
    self.setting.theme = themeIndex;
    [self applyThemeToRV:themeIndex];
    [self applyThemeToUI:themeIndex];
    [rv refresh];
}
...
-(void)movePIBox {
    double ppb = 0;
    PageInformation* pi = nil;
    
    if ([rv isGlobalPagination]) {
        int pib = slider.value;
    ...
    int ci = (int)pi.chapterIndex;
    NSString *caption;
    if ([rv isRTL]) {
        ci = (int)(pi.numberOfChaptersInBook-ci-1);
        caption = [rv.book getChapterTitle:ci];
    }else {
        caption = [NSString stringWithFormat:@"%@",pi.chapterTitle];
    }
    
    if (slider.value==slider.maximumValue && ![rv isRTL]) {
...
-(void)reflowableViewController:(ReflowableViewController*)rvc pageMoved:(PageInformation*)pageInformation{
    ...
    if ([rv isGlobalPagination]) {
        if (![rv isPaging]) {
            ...
            [slider setValue:pageInformation.pageIndexInBook];
        }
    }else {
        slider.value = ppb;
    }
...
//    NSLog(@"firstCharacterOffsetInPage %d   textLengthInPage %d",pageInformation.firstCharacterOffsetInPage,pageInformation.textLengthInPage);
...
-(void)enableControlAfterPagination {
    ...
    if ([rv isGlobalPagination]) {
        slider.maximumValue = [rv getNumberOfPagesInBook]-1;
        slider.minimumValue = 0;
        long globalPageIndex = [rv getPageIndexInBook];
        [slider setValue:globalPageIndex];
    }
...
-(void)dealloc {
    NSLog(@"dealloc called in BookViewController");
}
